...
final class OutputDeviceRouter {
    static let shared = OutputDeviceRouter()
    ...
    /// 列出所有「多输出设备」
    func listMultiOutputDevices() -> [OutputDevice] {
        return listAllOutputDevices().filter { $0.isMultiOutput }
    }

    /// 列出所有输出设备
    func listAllOutputDevices() -> [OutputDevice] {
    ...
    func currentDefaultOutput() -> OutputDevice? {
    ...
    /// 路由到指定设备；保存当前默认输出
    @discardableResult
    func route(to deviceID: AudioDeviceID) -> Bool {
