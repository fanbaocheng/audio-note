...
final class DownloadEngine {
    ...
    struct Config {
        var outputDir: URL = FileManager.default.urls(for: .downloadsDirectory, in: .userDomainMask).first!
            .appendingPathComponent("AudioNote/Downloads")
        var cookieSource: CookieSource = .none
        var ffmpegLocation: URL? = nil
    }
    ...
    /// 执行下载任务
    func execute(task: UniTask) async throws -> URL {
    ...
    static func siteHeaders(for url: String) -> [String] {
