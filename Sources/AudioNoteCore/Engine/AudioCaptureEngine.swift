...
public final class AudioCaptureEngine: ObservableObject {
    ...
    @Published public var rmsHistory: [Float] = []

    /// 系统音频候选设备（环回 / 虚拟 / 聚合）
    @Published public var availableSystemDevices: [AudioInputDevice] = []
    /// 麦克风候选设备（真实输入设备，排除环回）
    @Published public var availableMicDevices: [AudioInputDevice] = []
    /// 兼容旧 UI：与 availableSystemDevices 一致
    public var availableDevices: [AudioInputDevice] { availableSystemDevices }

    /// 当前选择的系统音频设备
    @Published public var selectedSystemDevice: AudioInputDevice? {
        didSet {
            if let uid = selectedSystemDevice?.uid {
                AppDefaults.shared.set(uid, forKey: "AudioNote.selectedSystemDeviceUID")
            }
        }
    }
    /// 当前选择的麦克风设备
    @Published public var selectedMicDevice: AudioInputDevice? {
        didSet {
            if let uid = selectedMicDevice?.uid {
                AppDefaults.shared.set(uid, forKey: "AudioNote.selectedMicDeviceUID")
            }
        }
    }
    ...
    @Published public var noDevicesFound = false

    /// 当前录制模式
    @Published public var recordingMode: RecordingMode = {
        if let raw = AppDefaults.shared.string(forKey: "AudioNote.recordingMode"),
           let mode = RecordingMode(rawValue: raw) {
            return mode
        }
        return .systemAudio
    }() {
        didSet { AppDefaults.shared.set(recordingMode.rawValue, forKey: "AudioNote.recordingMode") }
    }

    /// 混录时系统音频增益（0.0 ~ 2.0，UI 0%~200%）
    @Published public var mixSystemGain: Float = Float(AppDefaults.shared.object(forKey: "AudioNote.mixSystemGain") as? Double ?? 1.0) {
        didSet { AppDefaults.shared.set(Double(mixSystemGain), forKey: "AudioNote.mixSystemGain") }
    }
    /// 混录时麦克风增益（0.0 ~ 2.0）
    @Published public var mixMicGain: Float = Float(AppDefaults.shared.object(forKey: "AudioNote.mixMicGain") as? Double ?? 1.0) {
        didSet { AppDefaults.shared.set(Double(mixMicGain), forKey: "AudioNote.mixMicGain") }
    }
    /// 混录后是否保留原始两路（默认关，合并后删）
    @Published public var keepOriginalTracks: Bool = AppDefaults.shared.bool(forKey: "AudioNote.keepOriginalTracks") {
        didSet { AppDefaults.shared.set(keepOriginalTracks, forKey: "AudioNote.keepOriginalTracks") }
    }

    // MARK: - 输出路由（仅系统音频/混录模式生效）

    @Published public var autoRouteSystemOutput: Bool = true
    @Published public var preferredMultiOutputDeviceID: AudioDeviceID?
    @Published public var lastRouteMessage: String?
    @Published public var activeMultiOutputName: String?

    // MARK: - 静音自动停止

    @Published public var silenceSeconds: TimeInterval = 0
    @Published public var silenceAutoStopEnabled: Bool = true
    public var silenceLevelThreshold: Float = 0.02
    @Published public var silenceTimeoutSec: TimeInterval = 300
    ...
    @Published public private(set) var recordingFileURL: URL?
    ...
    // 兼容旧引用名
    public typealias LoopbackDevice = AudioInputDevice

    public init() {}

    // MARK: - 录音保存目录

    public static let recordingsDirectoryPathKey = "AudioNote.recordingsDirectoryPath"
    public static let recordingsDirectoryDefaultsChanged = Notification.Name("AudioNote.recordingsDirectoryChanged")
    ...
    public static func resolveRecordingsDirectory() -> URL {
        if let saved = AppDefaults.shared.string(forKey: recordingsDirectoryPathKey),
           !saved.isEmpty {
    ...
    public func setRecordingsDirectory(_ url: URL) {
        AppDefaults.shared.set(url.path, forKey: AudioCaptureEngine.recordingsDirectoryPathKey)
        try? FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
        objectWillChange.send()
        NotificationCenter.default.post(name: AudioCaptureEngine.recordingsDirectoryDefaultsChanged, object: nil)
    }

    public func resetRecordingsDirectory() {
        AppDefaults.shared.removeObject(forKey: AudioCaptureEngine.recordingsDirectoryPathKey)
        objectWillChange.send()
        NotificationCenter.default.post(name: AudioCaptureEngine.recordingsDirectoryDefaultsChanged, object: nil)
    }
    ...
    public func refreshDevices() {
        ...
        // 优先用 UserDefaults 中持久化的 UID 恢复（CLI/GUI 共享一致）
        let persistedSysUID = AppDefaults.shared.string(forKey: "AudioNote.selectedSystemDeviceUID")
        let persistedMicUID = AppDefaults.shared.string(forKey: "AudioNote.selectedMicDeviceUID")

        if selectedSystemDevice == nil || !systemDevices.contains(where: { $0.id == selectedSystemDevice?.id }) {
            if let uid = persistedSysUID, let m = systemDevices.first(where: { $0.uid == uid }) {
                selectedSystemDevice = m
            } else {
                selectedSystemDevice = systemDevices.first
            }
        }
        if selectedMicDevice == nil || !micDevices.contains(where: { $0.id == selectedMicDevice?.id }) {
            if let uid = persistedMicUID, let m = micDevices.first(where: { $0.uid == uid }) {
                selectedMicDevice = m
            } else {
                // 优先选系统默认输入设备
